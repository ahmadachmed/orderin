...
async function advanceToReady(page: Page, customerName: string, opts: { expectPin?: boolean } = {}) {
  ...
  await card.getByRole("button", { name: /→ confirmed/ }).click();
  await card.getByRole("button", { name: "Mark paid" }).click();
  await card.getByRole("button", { name: /→ brewing/ }).click();
  await card.getByRole("button", { name: /→ ready for pickup/ }).click();
...
test("full customer journey: order → READY → enter PIN → PICKED_UP", async ({ page }) => {
  ...
  await orderCard(page, customer).getByRole("button", { name: /→ picked up/ }).click();
...
test("wrong PIN → 403 → order stays READY_FOR_PICKUP", async ({ page }) => {
  ...
  await expect(card.getByText(/^PIN: \d{4}$/)).toBeVisible();
  await expect(card.getByRole("button", { name: /→ picked up/ })).toBeVisible();
});
...
test("legacy order (empty PIN) → transitions without a PIN", async ({ page }) => {
  ...
  await expect(orderCard(page, customer).getByText("Picked up")).toBeVisible();
});
